...
import { 
    ArrowLeft, Save, Upload, FileText, Cpu, Layers, 
...
export function AddProject({ onBack, onSuccess }: AddProjectProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Assemble payload packages with direct board form factors
    ...
    return (
        <div className="project-form-container">
            <header className="add-page-header">
                ...
                <div>
                    ...
                    <p style={{ margin: '4px 0 0 0', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                        Configure project metadata, silicon tapeout versions, and package board form factors
                    </p>
                </div>
            </header>
            ...
            <form onSubmit={handleSubmit} className="add-form" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    ...
                    <div className="project-meta-grid">
                        ...
                        <div className="form-group" style={{ margin: 0 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                <label htmlFor="project_key" style={{ fontSize: '0.85rem', fontWeight: 600, display: 'block', textAlign: 'left' }}>
                                    Project Key <span style={{ color: '#ef4444' }}>*</span>
                                </label>
                                <span style={{ fontSize: '0.75rem', color: isKeyManuallyEdited ? 'var(--accent)' : 'var(--text-muted)' }}>
                                    {isKeyManuallyEdited ? 'Manual' : 'Auto-derived'}
                                </span>
                            </div>
                            <div style={{ position: 'relative' }}>
                                <input 
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setProjectKey(e.target.value.toUpperCase().slice(0, 3));
                                        setAutoKeyError('');
                                    }} 
                                    ...
                                    style={{ 
                                        width: '100%', 
                                        padding: '0.75rem', 
                                        border: `1px solid ${keyBorderColor || 'var(--border)'}`, 
                                        borderRadius: '8px', 
                                        backgroundColor: 'var(--bg-secondary)', 
                                        color: keyTextColor || 'var(--text)',
                                        fontWeight: '700',
                                        letterSpacing: '2px',
                        ...
                        {/* Number Format */}
                        <div className="form-group" style={{ margin: 0 }}>
                            <label style={{ fontSize: '0.85rem', fontWeight: 600, marginBottom: '6px', display: 'block', textAlign: 'left' }}>
                                Board Number Format
                            </label>
                        ...
                        {/* Description */}
                        <div className="form-group" style={{ margin: 0 }}>
                            <label htmlFor="proj_desc" style={{ fontSize: '0.85rem', fontWeight: 600, marginBottom: '6px', display: 'block', textAlign: 'left' }}>
                                Description
                            </label>
                            <input 
                                id="proj_desc"
                ...
                {/* Error Banner */}
                {error && (
                    <div style={{ padding: '12px 16px', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: '8px', color: '#ef4444', fontSize: '0.85rem' }}>
                        {error}
                    </div>
                )}
                ...
                <div className="form-sticky-bar">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                        ...
                        {selectedFiles.length > 0 && (
                            <span style={{ fontSize: '0.78rem', background: 'rgba(234, 179, 8, 0.15)', color: '#eab308', padding: '3px 8px', borderRadius: '6px', border: '1px solid rgba(234, 179, 8, 0.3)' }}>
                                {selectedFiles.length} Doc{selectedFiles.length !== 1 ? 's' : ''}
                            </span>
                        )}
                    </div>
                    ...
                    <button 
                        type="submit" 
                        disabled={loading || !name || projectKey.length !== 3 || !!keyTextColor && keyTextColor === '#ef4444'} 
                        ...
                        style={{
                            ...
                            opacity: (loading || !name || projectKey.length !== 3 || keyTextColor === '#ef4444') ? 0.6 : 1
                        }}
                    >
                        <Save size={18} />
