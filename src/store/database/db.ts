...
const initDb = (): Promise<void> => {
    return new Promise((resolve) => {
        db.serialize(() => {
            ...
            db.run(`ALTER TABLE projects ADD COLUMN number_format TEXT DEFAULT 'decimal'`, (_err: any) => {
            ...
            db.run(`ALTER TABLE owners ADD COLUMN superuser INTEGER DEFAULT 0`, () => {
                ...
                db.get("SELECT COUNT(*) as count FROM owners WHERE superuser = 1", [], (errCount: any, row: any) => {
            ...
            db.run(`ALTER TABLE pcbs ADD COLUMN created_at DATETIME`, (err: any) => {
                if (err) {
                    console.log("Migration created_at log:", err.message);
                } else {
                    console.log("Migration created_at: successfully created column!");
                }
                db.run("UPDATE pcbs SET created_at = datetime('now') WHERE created_at IS NULL", (updateErr: any) => {
                    if (updateErr) {
